...
pub enum HashAlg {
    ...
    ///
    /// The image is converted to grayscale, scaled down to `hash_width x hash_height`,
    /// the median pixel value is calculated, and then the hash bits are generated by comparing
    /// the pixels of the descaled image to the mean.
    ///
    /// Further Reading:
    /// http://www.hackerfactor.com/blog/?/archives/432-Looks-Like-It.html
...
fn median_hash_u8<'a>(luma: &'a [u8]) -> impl Iterator<Item = bool> + 'a {
    ...
    let sorted_luma: &mut Vec<u8> = &mut luma.to_vec();
    sorted_luma.sort();

    if sorted_luma.len() % 2 == 1 {
        let lower = sorted_luma[(sorted_luma.len() - 1) / 2];
        let upper = sorted_luma[(sorted_luma.len() + 1) / 2];
        median = (lower + upper) / 2;
    } else {
        median = sorted_luma[sorted_luma.len() / 2];
    }

    luma.iter().map(move |&x| x >= median)
}

fn median_hash_f32<'a>(luma: &'a [f32]) -> impl Iterator<Item = bool> + 'a {
    let median;

    let sorted_luma: &mut Vec<f32> = &mut luma.to_vec();
